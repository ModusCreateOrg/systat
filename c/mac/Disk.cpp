...
void DiskStats::diff(DiskStats *newer, DiskStats *older) {
  ...
  this->total_written_bytes = newer->total_written_bytes - older->total_written_bytes;
...
int Disk::record_all_devices() {
  ...
  CFRetain(match);
  status = IOServiceAddMatchingNotification(notifyPort, kIOFirstMatchNotification, match, &xrecord_drivelist, NULL,
                                            &drivelist);
  ...
  status = IOServiceAddMatchingNotification(notifyPort, kIOTerminatedNotification, match, &xremove_drivelist, NULL,
                                            &drivelist);
...
void Disk::copy(std::map<std::string, DiskStats *> &dst, std::map<std::string, DiskStats *> &src) {
...
uint16_t Disk::print(bool test) {
  uint16_t count = 0;
  if (!test) {
    console.inverseln("%-16s %13s %13s %13s %13s %13s", "DISK ACTIVITY", "Block Size", "Total Bytes", "Read Bytes",
                      "Write Bytes", "Transfers");
  }
  count++;
  for (const auto &kv : this->delta) {
    if (!test) {
      DiskStats *stats = (DiskStats *) kv.second;
      console.println("%-16s %'13ld %'13ld %'13ld %'13ld %'13ld", stats->name, stats->blocksize, stats->total_bytes,
                      stats->total_read_bytes,
                      stats->total_written_bytes, stats->total_transfers);
    }
    count++;
  }
  return count;
